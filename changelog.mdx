...
- **A broad speed pass.** The dashboard, opening a project, and chat are all significantly faster — heavy views load roughly 30% quicker, and the hottest screens paint instantly from cache.
...
- **Effort levels.** Choose how deep each sprint goes with `/effort` — **Vibe** (fast direct pass, now the default for quick work), **Todo** (task-list loop), or **Build** (full research → code → verify pipeline). See [Build Modes & Effort](/coding-agent/building/build-modes).
