...
Every session is an **isolated branch of your project**. It gets its own `predev/<task-slug>` git branch, its own chat thread, and its own sandbox — so an experiment in one session can't break another.
...
Sessions are multiplayer. Presence avatars on each tab show who else is viewing that session right now, and session changes (create, rename, archive, promote) sync live to everyone in the project. Double-click a tab to rename it.
