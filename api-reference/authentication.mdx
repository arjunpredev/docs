...
The same key authenticates the [AI Gateway](/ai-gateway/overview) at `https://api.pre.dev/v1`. OpenAI-compatible SDKs send it as a Bearer token and the Anthropic SDK sends it as `x-api-key`; both are accepted. The key is also shown in the dashboard under **Integrations → API Keys**, and projects built on pre.dev receive it as `PREDEV_API_KEY` automatically. See [API key](/ai-gateway/api-key).
