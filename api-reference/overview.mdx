...
The [AI Gateway](/ai-gateway/overview) is the one API mounted under a prefix: **`https://api.pre.dev/v1`** proxies every OpenRouter endpoint (`/v1/chat/completions`, `/v1/messages`, `/v1/embeddings`, media, files, and a priced model catalog) with the same key and credits. Point the stock OpenAI, Vercel AI, or Anthropic SDK at that base URL; see the [quickstart](/ai-gateway/quickstart) and [pricing](/ai-gateway/pricing).
