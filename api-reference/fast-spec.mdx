...
2. **Automatic Deduplication:** URLs are automatically deduplicated by hostname (e.g., multiple `stripe.com` URLs result in one archive)
3. **Graceful Degradation:** If documentation scraping fails, spec generation still completes successfully
4. **Organized Archives:** Each platform gets its own ZIP with hierarchical folder structure based on the documentation site
...
**Note:** The two `stripe.com` URLs were automatically deduplicated into a single archive.
...
**Don't:**
- ❌ Provide the same domain multiple times (automatic deduplication will handle it)
