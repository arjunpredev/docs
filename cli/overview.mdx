...
<Steps>
  <Step title="Install">
    ...
    curl -fsSL https://pre.dev/install | bash
    ```
    The installer launches `predev` for you as soon as it finishes — one command and you're in the agent. It also adds `predev` to your PATH for next time (set `PREDEV_NO_AUTORUN=1` to skip the auto-launch, e.g. in provisioning scripts).
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Credits and models" icon="sliders">
    `/balance` shows remaining credits; `/model`, `/effort`, and `/pro` control which models do the work. If you run out mid-task, the CLI opens your billing page — top up and it resumes the interrupted work automatically.
  </Card>
</CardGroup>
