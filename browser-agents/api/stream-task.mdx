...
description: "GET /browser-agent/{id}/stream — Server-Sent Events for a batch that's already running."
---

# GET /browser-agent/:id/stream

Attach a live Server-Sent Events stream to an in-progress batch. You get an
immediate snapshot, then per-step events as they happen. The stream closes
itself when the batch completes or fails.
...
| Event | Payload | When |
|---|---|---|
| `snapshot` | Full batch result (with events) | Immediately on connect |
...
| `done` | Final batch state | Batch completed or failed, then the stream closes |

## Errors

| Status | Meaning |
|---|---|
| `404` | Batch not found (or not yours) |
