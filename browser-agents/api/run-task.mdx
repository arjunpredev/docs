---
title: "Run a Task"
api: "POST https://api.pre.dev/api/v1/browser-agents"
openapi: "POST /api/v1/browser-agents"
...
| `Idempotency-Key` | ❌ | Deduplicates retries for this `(user, key)` pair within 24h. Returns the original run instead of creating a duplicate. |
...
| `idempotencyKey` | `string` | ❌ | Body-level alternative to the `Idempotency-Key` header. |
...
Returns immediately with a `batchId`. Poll [`GET /api/v1/browser-agents/:id`](/browser-agents/api/task-status) for progress.
...
Network retries can cause duplicate submissions. Pass an `Idempotency-Key` (header or body) to ensure the same key within 24h returns the original run — you won't be double-charged.

```bash
curl -X POST https://api.pre.dev/api/v1/browser-agents \
  -H "Authorization: Bearer $PREDEV_API_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: import-run-2026-04-15-042" \
  -d '{ "tasks": [{ "url": "https://example.com", "instruction": "Extract the heading." }] }'
```

Keys are scoped per API-key owner. Use a key that uniquely identifies the *intent* of the submission (e.g., a DB row id, a job id), not a random UUID regenerated on retry.
...
export class BrowserAgents {
  ...
  async run(req: BatchRequest, idempotencyKey?: string): Promise<BatchResult> {
    const headers: Record<string, string> = this.headers();
    if (idempotencyKey) headers["Idempotency-Key"] = idempotencyKey;
    const res = await fetch(this.base, { method: "POST", headers, body: JSON.stringify(req) });
    if (!res.ok) throw new Error(`${res.status} ${await res.text()}`);
    return res.json();
  }
}
