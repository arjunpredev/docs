...
- Browser billing uses **$0.10 per credit**. A successful task's `creditsUsed` is its full charge. A failed task reports `creditsUsed: 0` even though its 0.1-credit floor was charged, so `totalCreditsUsed` does not include the floors of failed tasks. Treat both fields as recorded usage, not a forecast or a reserved amount.
