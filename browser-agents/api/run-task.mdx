...
- **Max in-flight tasks per user:** varies by plan (5 on trial, up to 150 on Enterprise). Check yours with [`GET /browser-agent-status`](/browser-agents/api/queue-status)
