...
```bash
export PREDEV_API_KEY=...   # from pre.dev/projects/key
...
On first use a browser tab opens — sign in to pre.dev to authorize.
...
3. An AI planner drives the browser through navigation, actions, and
   extraction.
4. The result streams back as structured JSON, validated against your
   `output` schema.
...
For long-running task sets, flip to async:
