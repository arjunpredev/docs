...
description: "Swap Browser Use Cloud for pre.dev Browser Agents — same task shape, 100/100 vs 93/100 pass rate, ~2.8× cheaper AND ~4× faster."
...
On our public [benchmark](/browser-agents/overview#benchmark) (June 2026 run), pre.dev passes **100/100** vs Browser Use Cloud's **93/100** — and is **~2.8× cheaper and ~4× faster** ($0.0143 vs $0.0405 per task, 7.9s vs 32.0s avg).
...
Grab your pre.dev key at [pre.dev/projects/key](https://pre.dev/projects/key).
...
const responses = await Promise.all(
  urls.map((url) =>
    client.run(`Extract the top 5 story titles from ${url}. Return JSON { titles: string[] }.`, { model: 'bu-mini' }),
  ),
);
...
async def one(url):
    r = await client.run(f"Extract the top 5 story titles from {url}. Return JSON {{titles: string[]}}.", model="bu-mini")
...
```ts
const handle = client.run(task, { model: 'bu-mini' });
...
handle = client.run(task, model="bu-mini")
...
output, ~2.8× cheaper AND ~4× faster at 100/100 pass on the public
benchmark (vs Browser Use's 93/100). Docs:
...
3. **Rewrite calls** per these rules. Full mapping at
   ...
   | `client.run(taskStr, { model })` | `client.browserAgent([{ url, instruction, output }])` |
   ...
   | Python `client.run(task, model=...)` | `client.browser_agent([{...}])` |
...
5. **Unsupported features — leave TODOs, don't delete.** For any of:
   ...
   `// TODO: pre.dev equivalent not yet available — see https://docs.pre.dev/browser-agents/migrate-from-browser-use`
