...
Both services pass 100/100 on our public [benchmark](/browser-agents/overview#benchmark) — pre.dev is **12× cheaper** ($0.0103 vs $0.1222 per task).
...
```ts
import { Predev } from '@predotdev/sdk';

const client = new Predev({ apiKey: process.env.PREDEV_API_KEY! });

const { results } = await client.browserAgents.run({
  tasks: [{
...
const data = results[0].data; // { heading: 'Example Domain' }
```
</Tab>
<Tab title="Python">
**Before — Browser Use**
...
result = client.browser_agents.run(
    tasks=[{
        "url": "https://example.com",
        "instruction": "Extract the H1.",
        "output": {
...
- **`tasks` is always an array** — one endpoint covers 1 task or 1,000.
...
If you were polling Browser Use by session id, pre.dev has a direct equivalent: set `async: true` on submit, then `GET /browser-agent/:id`.
...
```ts
const { id } = await client.browserAgents.run({
  async: true,
  tasks: [{ url, instruction, output }],
});

while (true) {
  const batch = await client.browserAgents.get(id);
...
batch = client.browser_agents.run(async_=True, tasks=[{"url": url, "instruction": instruction, "output": output}])

while True:
    status = client.browser_agents.get(batch.id)
...
See [Task Status](/browser-agents/api/task-status) for the full polling contract (including `includeEvents=true` for the per-step timeline).

---

## What's different / not yet supported

pre.dev Browser Agents covers the run-a-task-and-get-structured-JSON flow that ~95% of Browser Use code does. A few Browser Use features don't have direct equivalents yet:

- **Persistent sessions** — keeping a browser warm across requests for login-gated flows. Workaround: pass credentials in each task's `input` field; the agent will log in fresh each run.
- **Agent profiles** — reusable personas with tools/settings. Workaround: put the persona in each task's `instruction`.
- **Schedules** — recurring runs. Workaround: cron + a 10-line script that calls `/browser-agent` on your own scheduler (GitHub Actions, Render cron, Cloudflare Workers cron).
- **Raw Playwright access** — Browser Use lets you hop into the session with raw Playwright. pre.dev abstracts this away entirely; if you need raw browser control, stick with Browser Use for that subset.

The Claude prompt below will flag any usage of these features with `// TODO: pre.dev equivalent not yet available` so you can migrate the common paths automatically and hand-handle the rest.
...
2. **Swap the SDK.**
   - Node/TS/Bun: replace `browser-use-sdk` dependency with
     `@predotdev/sdk` in `package.json`, `bun.lockb`, `yarn.lock`,
     or `pnpm-lock.yaml` (let the user run the install).
   - Python: replace `browser-use-sdk` with `predev` in
...
3. **Rewrite calls** per these rules. Full mapping at
   ...
   | Browser Use | pre.dev |
   |---|---|
   | `new BrowserUse({ apiKey })` | `new Predev({ apiKey })` |
   | `client.run(taskStr, { llm })` | `client.browserAgents.run({ tasks: [{ url, instruction, output }] })` |
   | Python `BrowserUse(api_key=...)` | `Predev(api_key=...)` |
   | Python `client.run(task, llm=...)` | `client.browser_agents.run(tasks=[{...}])` |
   | `response.output` (JSON string) | `results[0].data` (already parsed) |
   | Embedded URL in task string | first-class `url` field |
   | Zod/Pydantic schema | JSON Schema in `output` field |
   | Parallel via `Promise.all(client.run(...))` | single call with `tasks: [...]` array + `concurrency` |
   | `handle.sessionId` + session polling | `{ async: true }` + `client.browserAgents.get(id)` |

4. **Env vars.** Rename `BROWSER_USE_API_KEY` → `PREDEV_API_KEY`
   everywhere (code, env files, CI, docker). Base URL is
   `https://api.pre.dev` if it's referenced explicitly anywhere.
...
7. **Report.** At the end, print:
   - Files changed (grouped by purpose: imports, env, call sites).
   - Count of migrated calls.
   - Count of TODO-comments left for unsupported features.
   - Any test/typecheck failures you couldn't fix, with the exact
     file:line and why.

## Constraints

- **Don't delete** any original code until the replacement is verified
  working. If you're unsure of a mapping, leave the old call with a
  TODO and surface it in the final report.
- **Don't guess** the pre.dev SDK name if `@predotdev/sdk` or `predev`
  isn't on the registry your project uses — stop and ask.
