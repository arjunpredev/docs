...
```python
result = client.list_browser_agents(limit=20, skip=0, status="completed")
# {"batches": [...], "total": 132, "hasMore": True}
```
...
```python
q = client.browser_agent_status()
# {"userId": ..., "running": 12, "claimed": 3, "pending": 40, "total": 55, "cap": 100}
if q["total"] < q["cap"] - 10:
    client.browser_agent(next_chunk, run_async=True)
```
