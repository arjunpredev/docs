...
```ts
const { batches, total, hasMore } = await client.listBrowserAgents({
  limit: 20,          // optional
  skip: 0,            // optional
  status: 'completed' // optional: 'processing' | 'completed'
});
```
...
```ts
const q = await client.browserAgentStatus();
// { userId, running, claimed, pending, total, cap }
if (q.total < q.cap - 10) {
  await client.browserAgent(nextChunk, { async: true });
}
```
