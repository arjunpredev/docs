...
<Card title="Grab a key" icon="key" href="https://pre.dev/projects/key">
  One click to copy your API key and see remaining credits.
</Card>
...
<Tabs>
  <Tab title="curl">
    ...
    You'll get back a structured spec with milestones, user stories, and architecture — in about a minute.
  </Tab>
  ...
  <Tab title="Python SDK">
    ...
    spec = client.fast_spec(input_text="Build a task management app with team collaboration")
    print(spec)
    ```
...
For a richer breakdown, use [`/deep-spec`](/architect-agent/api/deep-spec) instead — it adds granular subtasks and per-task acceptance criteria (~3–5 minutes).
