...
class SpecResponse:
    ...
    architectureInfographicUrl: Optional[str] = None  # Rendered architecture infographic
    zippedDocsUrls: Optional[List[dict]] = None   # Downloadable doc bundles
    userFlowGraph: Optional[dict] = None          # User flow nodes + edges
    architectureGraph: Optional[dict] = None      # System architecture nodes + edges
    enrichedTechStack: Optional[List[dict]] = None  # Tech choices with useFor/reason
    creditsUsed: Optional[float] = None           # Credits consumed (live during processing)
...
spec = predev.fast_spec(
    input_text="Build a task management app with team collaboration"
)
print(spec.codingAgentSpecMarkdown)
```
...
All SDK errors inherit from `PredevAPIError`. Typed subclasses map to HTTP status codes so you can handle billing and rate-limit cases explicitly:
...
| `PredevAPIError` | other | Any other API error (has `.status_code`) |
...
try:
    spec = predev.fast_spec(input_text="Build a task management app")
except AuthenticationError:
    print("Check your API key at pre.dev/projects/key")
...
<Note>
Typed exceptions require `predev-api` >= 1.1.0.
</Note>
