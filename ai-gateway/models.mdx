---
title: "Models"
description: "Use any OpenRouter model id and read credit prices from the catalog."
---

The gateway serves every model OpenRouter serves, under its exact OpenRouter id. There is no pre.dev catalog to learn: if OpenRouter lists it, `model: "<author>/<slug>"` works here.
...
Variants pass through unchanged: `deepseek/deepseek-v4.1-flash:free`, `anthropic/claude-sonnet-5:nitro`, `google/gemini-3.8-flash:online`, `openai/gpt-5.6-luna:batch`. Their meaning is defined by [OpenRouter](https://openrouter.ai/docs).
...
Each row is OpenRouter's model row plus a `predev` object with prices already converted to credits:
...
The catalog is a mirror of OpenRouter refreshed every 15 minutes. The `x-predev-catalog-age` response header reports seconds since the last refresh. Catalog reads are free and not rate-limited.
...
An id OpenRouter does not recognize returns OpenRouter's own 4xx error with its original status; the gateway does not rewrite it. See [errors](/ai-gateway/api/errors).
