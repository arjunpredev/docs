...
The job is charged once it finishes, not at submission: on the first status poll that reports completion, on the `/content` download if you never polled, or by pre.dev afterwards if you stopped polling. Poll until the status is terminal (completed or failed) before reading `/content`; the status body reports the job `id` and its current state.
