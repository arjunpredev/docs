---
title: "Model catalog"
description: "Read-only catalog endpoints with credit prices, mirrored from OpenRouter every 15 minutes."
---

The catalog is a mirror of OpenRouter's model and provider lists with pre.dev credit prices added. Reads are free, not rate-limited, and do not need credits.

| Method | Path | Returns |
| --- | --- | --- |
| GET | `/v1/models` | All models, OpenRouter's row plus `predev` |
...
Each model row carries prices already converted with the [pricing formula](/ai-gateway/pricing):
...
The numbers above illustrate the conversion; read the live row for current prices. Every other field in the row is OpenRouter's and is described in the [OpenRouter docs](https://openrouter.ai/docs).

## Freshness

`x-predev-catalog-age` on every catalog response is the number of seconds since the mirror last refreshed from OpenRouter. The mirror refreshes every 15 minutes, so a model OpenRouter added a moment ago can be callable before it appears in the list. Calling a model id that is not in the mirror still works if OpenRouter serves it.
...
Use this with `provider` preferences to pin or exclude specific upstreams. See [routing and fallbacks](/ai-gateway/routing-and-fallbacks).
