---
title: "Errors"
description: "Gateway error codes in the OpenAI error shape, plus pass-through of OpenRouter errors."
---

Errors raised by the gateway use the OpenAI error shape with an extra `predev` object. Errors raised by OpenRouter, for example an invalid model id, pass through unchanged with their original status and body.
...
| `upstream_unavailable` | `502` | OpenRouter did not answer | | Retry with backoff; consider `models` fallbacks |
...
OpenRouter's own 4xx responses keep their status and body. An unknown model id, a malformed request, or a model that rejects a parameter all arrive as OpenRouter returns them. Check the model id against [`GET /v1/models`](/ai-gateway/api/models) first.
