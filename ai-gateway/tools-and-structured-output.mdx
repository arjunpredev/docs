...
description: "Tool calling and JSON-schema output pass through to OpenRouter unchanged."
---

`tools`, `tool_choice`, `parallel_tool_calls`, `response_format`, and `structured_outputs` are forwarded as-is. Model support for each is OpenRouter's; check the model's row in [`GET /v1/models`](/ai-gateway/api/models) and the [OpenRouter docs](https://openrouter.ai/docs).
