...
**Don't:**
- ❌ Reference multiple URLs from the same domain (automatic deduplication handles this)
...
1. **Deduplicates URLs:** Multiple URLs from the same domain → single comprehensive archive
2. **Handles Failures Gracefully:** Spec generation completes even if documentation scraping fails
3. **Organizes by Platform:** Each documentation source gets its own well-structured archive
4. **Runs in Parallel:** Documentation scraping doesn't slow down spec generation
