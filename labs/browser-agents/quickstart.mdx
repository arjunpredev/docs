...
- completes in ~3 seconds warm, ~8s cold
...
const result = await client.browserAgents.run({
  tasks: [
    {
      url: 'https://example.com',
      instruction: 'Extract the heading.',
...
Full MCP reference: [MCP Tool](/labs/browser-agents/mcp-tool)
...
4. Result streams back as structured JSON, validated against your
   `output` schema.
5. Billed per task (~$0.011 on simple tasks, 10× margin on long tasks
   — see [Pricing](/labs/browser-agents/pricing)).
...
# Poll for the batch
curl https://api.pre.dev/api/v1/browser-agents/$BATCH_ID \
  -H "Authorization: Bearer $PREDEV_API_KEY"
```
...
Full wire format: [REST API reference](/labs/browser-agents/rest-api)
